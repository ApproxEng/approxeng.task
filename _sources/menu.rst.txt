...
Returning a Value from a Menu
*****************************

If a task returns :class:`~approxeng.task.TaskStop` when called, the :func:`~approxeng.task.run` function will return
the value wrapped in the :class:`~approxeng.task.TaskStop`. If we wanted to use the menu system to get user input we
could therefore define a set of :class:`~approxeng.task.Task` instances, or equivalent functions decorated with ``task``
to return different values, and then reference those tasks from the menu structure. This would be very clunky though,
and it's a sufficiently common case that the library supports it directly.

If you create a menu such as:
...
This will automatically create two new tasks, each of which simply returns a :class:`~approxeng.task.TaskStop` wrapping
the two strings ``Foo`` and ``Bar`` respectively, and rewrites the menu structure so that these new tasks are called
when those options are selected.

These three kinds of things (tasks, sub-menus, and return values) can be freely mixed in any menu, the library will do
the appropriate thing in each case.
